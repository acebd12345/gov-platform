...
interface HomepageContentProps {
  ...
  newsItems: PageSummary[];
  categories: CategoryNode[];
}

export function HomepageContent({ initialConfig, newsItems, categories }: HomepageContentProps) {
  const [cfg, setCfg] = useState<HomepageConfig>(initialConfig);
  ...
  useEffect(() => {
    ...
    function onMessage(e: MessageEvent) {
      ...
      if (data.draft?.homepageConfig) {
        setCfg(data.draft.homepageConfig);
      }
    }
  ...
  // 業務專區：勾了 businessFromCategories 就用 categories；否則用 cfg.businessCards
  const CATEGORY_ICONS = ['heritage', 'arts', 'creative', 'book', 'film', 'exchange', 'subsidy'];
  const businessFromCats = cfg.businessFromCategories
    ? categories.map((c, i) => ({
        title: c.name,
        description: '',
              ...
              {isOn(cfg, 'news') && (
                <div>
                  {newsTabs && newsTabs.length > 0 ? (
                    <NewsTabs tabs={newsTabs} items={news} hrefPrefix="/news" />
                  ) : (
                    <>
